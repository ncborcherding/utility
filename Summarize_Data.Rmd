...
```{r include=FALSE}
"%!in%" <- Negate("%in%")
full.data <- unique(list.files("./data/processedData/individualSeurat/rds", pattern = ".rds"))
...
for (i in seq_along(full.data)){
    tmp <- readRDS(paste0("./data/processedData/individualSeurat/rds/", full.data[i]))
    ...
    colnames.switch <- colnames(tmp[[]])
    check.1 <- which(colnames.switch == "functional.cluster")
    check.2 <- which(colnames.switch == "PT.annot")
    if(length(check.1) > 0) {
      col.pos <- check.1
    }else if (length(check.2) > 0) {
      col.pos <- check.2
    }
    full.data.summary
    tmp$filter <- "No"
    
    tmp$filter <- ifelse(!is.na(tmp$CTaa) & !is.na(tmp[[]][,col.pos]), "Yes", "No") 
