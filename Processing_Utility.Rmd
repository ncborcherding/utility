...
for (j in seq_along(tmp)) {
  ...
  saveRDS(com.res1, file = paste0("./annotation/singler/",  samples[i], "_", names(tmp)[j], "_HPCA.singler_output.rds"))
  com.res2 <- SingleR(tmp.2, ref=DICE, labels=DICE$label.fine, assay.type.test=1)
  saveRDS(com.res2, file = paste0("./annotation/singler/",  samples[i], "_", names(tmp)[j], "_DICE.singler_output.rds"))
...
sing.list <- list.files("./annotation/singler/", pattern = ".rds")
HPCA.ann <- NULL
dice.ann <- NULL


for ( i in seq_along(sing.list)) {
  ann <- readRDS(paste0("./annotation/singler/", sing.list[i]))
...
dir.create("./annotation/projectil/")
samples <- unique(list$Sample)
for (i in seq_along(samples)) {
  tmp <- subset(list, Sample == samples[i])
  tmp <- SplitObject(tmp, split.by = "Type")
  for (j in seq_along(tmp)) {
    ...
    saveRDS(meta, file = paste0("./annotation/projectil/",  samples[i], "_", names(tmp)[j],  "_output.rds"))
    rm(query.projected)
  }
}
...
pro.list <- list.files("./annotation/projectil/", pattern = ".rds")
proj.annot <- NULL
for ( i in seq_along(pro.list)) {
  ann <- readRDS(paste0("./annotation/projectil/", pro.list[i]))
...
DimPlot(list, cells = rownames(list[[]])[sample(nrow(list[[]]), 20000)], group.by = "consensus.major") + 
  scale_color_manual(values = mycolors) + 
  theme(plot.title = element_blank())
ggsave("./UMAP/major.consensus.pdf", height = 3.5, width = 5)

mycolors <- colorRampPalette(brewer.pal(8, "Set1"))(length(unique(list$DICE.pruned.labels)))
DimPlot(list, cells = rownames(list[[]])[sample(nrow(list[[]]), 20000)], group.by = "DICE.pruned.labels") + 
  scale_color_manual(values = mycolors) + 
  theme(plot.title = element_blank())
ggsave("./UMAP/DICE.pdf", height = 3.5, width = 6)

DimPlot(list, cells = rownames(list[[]])[sample(nrow(list[[]]), 20000)], group.by = "consensus.Tcell") + 
  scale_color_manual(values = mycolors[c(1,3,5)], na.value="grey") + 
  theme(plot.title = element_blank())
ggsave("./UMAP/Tcell.consensus.pdf", height = 3.5, width = 4.25)
...
#This prevents the issue with overlap that can be seen in DotPlots, reduces the size of the subsequent visualization files, and as UMAP is based on nearest neighbor it make more sense to incorporate neighbors while visualizing (at least to me, I have had a reviewer or two that have not been so inclined)
...
set.seed(123)
mycolors <- colorRampPalette(brewer.pal(8, "Set1"))(length(unique(list$Tissue)))
plot1 <- DimPlot(list, cells = rownames(list[[]])[sample(nrow(list[[]]), 20000)], group.by = "Tissue") + 
  theme_void() +
  scale_color_manual(values = mycolors) + 
  theme(plot.title = element_blank(), 
...
plot2 <- DimPlot(list, cells = rownames(list[[]])[sample(nrow(list[[]]), 20000)], group.by = "DICE.pruned.labels") + 
  theme_void()+
  scale_color_manual(values = mycolors) + 
  theme(plot.title = element_blank(), 
        legend.text=element_text(size=6)) 
