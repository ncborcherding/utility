...
object.list <- lapply(files, function(x) {
                      ...
                      features <- rownames(obj@assays$RNA$counts)
                      pos <- which(features %in% gene.symbols$Approved.Symbol)
                      not.pos <- seq_len(length(features))[-pos]
                      
                      keep.values <- c()
                      remove <- c()
                      for(i in seq_along(not.pos))
                        if(features[not.pos[i]] %in% gene.symbols$Symbol) {
                          new.gene <- gene.symbols$Approved.Symbol[gene.symbols$Symbol %in% features[not.pos[i]]]
                          keep.values <- c(keep.values,new.gene)
                        } else {
                          remove <- c(remove,i)
                        }
                      not.pos2 <- not.pos[-remove]
...
features.to.use <- features.to.use[1:4000]
